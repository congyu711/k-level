...
namespace klevel
{
  ...
  pair<vector<int>,vector<pair<double, int>>> klevel_with_init_set(int k, vector<line> *lines)
  {
    // std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
    vector<pair<double, int>> res;
    kineticPriorityQueue<double, less<double>> upper(k, lines);
    ...
    pair<vector<int>,vector<pair<double, int>>> return_pair{init,res};
    return return_pair;
  }
}